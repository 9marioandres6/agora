...
const routes: Routes = [
  ...
  {
    path: 'profile/:userId',
    loadComponent: () => import('./public-profile/public-profile.component').then(c => c.PublicProfileComponent),
    canActivate: [authGuard]
  },
  {
    path: 'settings',
  ...
  {
    path: 'location',
    loadComponent: () => import('./location/location.page').then(c => c.LocationPage),
    canActivate: [authGuard]
  },
  {
    path: 'login',
